...
public class MapDbIndex implements DisposableBean, Index {
    ...
    @Override
    public void registerAsset(IndexPointer asset, IndexPointer surrogate, List<KnowledgeAssetType> types, List<Annotation> annotations, String name, String description) {
        types.stream().forEach(type -> {
            HTreeMap<String, Set<IndexPointer>> map = (HTreeMap<String, Set<IndexPointer>>) this.db.hashMap(BY_TYPE_MAP).createOrOpen();
        ...
        this.registerAnnotations(asset, annotations.stream().map(annotation -> (SimpleAnnotation) annotation).collect(Collectors.toSet()));
        this.registerDescriptiveMetadata(asset, name, description, types.stream().map((type) -> type.getRef()).collect(Collectors.toSet()));
    }

    private void registerLatestAsset(IndexPointer assetPointer) {
        HTreeMap<String, IndexPointer> map = (HTreeMap<String, IndexPointer>) this.db.hashMap(LATEST_ASSET_MAP).createOrOpen();
    ...
    public void registerArtifactToAsset(IndexPointer assetPointer, IndexPointer artifact) {
        HTreeMap<IndexPointer, Set<IndexPointer>> map = (HTreeMap<IndexPointer, Set<IndexPointer>>) this.db.hashMap(ASSET_TO_ARTIFACT_MAP).createOrOpen();
    ...
    public void registerSurrogateToAsset(IndexPointer assetPointer, IndexPointer surrogate) {
        HTreeMap<IndexPointer, IndexPointer> map = (HTreeMap<IndexPointer, IndexPointer>) this.db.hashMap(ASSET_TO_SURROGATE_MAP).createOrOpen();
    ...
    public void registerLocation(IndexPointer pointer, String href) {
        HTreeMap<IndexPointer, String> map = (HTreeMap<IndexPointer, String>) this.db.hashMap(ID_TO_LOCATION_MAP).createOrOpen();
    ...
    public String getLocation(IndexPointer pointer) {
        ...
        if(map.containsKey(pointer)) {
            ...
            throw new RuntimeException("Location of: " + pointer.getId() + ", " + pointer.getVersion() + " not found.");
        }
    }
    ...
    public Set<IndexPointer> getAssetIdsByType(String assetType) {
        ...
        if(StringUtils.isNotBlank(assetType)) {
            ...
            return this.db.hashSet(ASSETS_SET).createOrOpen().stream().map(p -> (IndexPointer) p).collect(Collectors.toSet());
        }
    }

    @Override
    public void registerAnnotations(IndexPointer pointer, Set<SimpleAnnotation> annotations) {
        HTreeMap<String, Set<IndexPointer>> map = (HTreeMap<String, Set<IndexPointer>>) this.db.hashMap(BY_ANNOTATION_MAP).createOrOpen();

        HTreeMap<String, Set<String>> typeMap = (HTreeMap<String, Set<String>>) this.db.hashMap(BY_ANNOTATION_TYPE_MAP).createOrOpen();
    ...
    @Override
    public void registerDescriptiveMetadata(IndexPointer pointer, String name, String description, Set<URI> types) {
        HTreeMap<IndexPointer, DescriptiveMetadata> map = (HTreeMap<IndexPointer, DescriptiveMetadata>) this.db.hashMap(DESCRIPTIVE_METADATA_MAP).createOrOpen();
